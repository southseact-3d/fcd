...
__title__ = "PartDesign.BOPTools.ShapeMerge module"
__author__ = "DeepSOIC"
...
def findSharedElements(shape_list, element_extractor):
    ...
    shared_elements = {HashableShape(sh) for sh in element_extractor(shape_list[0])}
    for shape in shape_list[1:]:
        shared_elements.intersection_update(
...
def splitIntoGroupsBySharing(list_of_shapes, element_extractor, split_connections=None):
    ...
    split_connections = {HashableShape(element) for element in split_connections or ()}
    ...
    for shape in list_of_shapes:
        shape_elements = {HashableShape(element) for element in element_extractor(shape)}
        shape_elements.difference_update(split_connections)
        # search if shape is connected to any groups
        connected_to = []
        not_in_connected_to = []
        for iGroup, group in enumerate(groups):
            if shape_elements.intersection(group[1]):
...
def mergeSolids(
    list_of_solids_compsolids, flag_single=False, split_connections=None, bool_compsolid=False
):
...
def mergeShells(list_of_faces_shells, flag_single=False, split_connections=None):
    faces = []
...
def mergeWires(list_of_edges_wires, flag_single=False, split_connections=None):
    edges = []
...
def mergeVertices(list_of_vertices, flag_single=False, split_connections=None):
...
def mergeShapes(list_of_shapes, flag_single=False, split_connections=None, bool_compsolid=False):
...
def removeDuplicates(list_of_shapes):
    ...
    for sh in list_of_shapes:
        hashed_shape = HashableShape(sh)
        if hashed_shape not in hashes:
            new_list.append(sh)
            hashes.add(hashed_shape)
    return new_list
...
def dimensionOfShapes(list_of_shapes):
    ...
    for sh in list_of_shapes:
        ...
        if iDim != dim:
            raise TypeError(
                "Shapes are of different dimensions ({t1} and {t2}), "
                "and cannot be merged or compared.".format(
                    t1=list_of_shapes[0].ShapeType,
                    t2=sht,
                )
            )
    return dim
