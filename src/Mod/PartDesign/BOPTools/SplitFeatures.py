...
__title__ = "PartDesign.BOPTools.SplitFeatures module"
__author__ = "DeepSOIC"
...
def cmdCreateBooleanFragmentsFeature(name, mode):
    ...
    FreeCADGui.addModule("PartDesign.BOPTools.SplitFeatures")
    FreeCADGui.doCommand(
        "j = PartDesign.BOPTools.SplitFeatures.makeBooleanFragments(name='{name}')".format(name=name)
    )
    FreeCADGui.doCommand(
...
def cmdCreateSliceFeature(name, mode, transaction=True):
    ...
    FreeCADGui.addModule("PartDesign.BOPTools.SplitFeatures")
    FreeCADGui.doCommand("f = PartDesign.BOPTools.SplitFeatures.makeSlice(name='{name}')".format(name=name))
    FreeCADGui.doCommand(
        "f.Base = {sel}[0]\n"
...
def cmdSliceApart():
    ...
    if made:
        FreeCADGui.addModule("PartDesign.CompoundTools.Explode")
        FreeCADGui.doCommand("PartDesign.CompoundTools.Explode.explodeCompound(f)")
...
def cmdCreateXORFeature(name):
    ...
    FreeCADGui.addModule("PartDesign.BOPTools.SplitFeatures")
    FreeCADGui.doCommand("j = PartDesign.BOPTools.SplitFeatures.makeXOR(name='{name}')".format(name=name))
    FreeCADGui.doCommand(
        "j.Objects = {sel}".format(
            sel="[" + ", ".join(["App.ActiveDocument." + so.Object.Name for so in sel]) + "]"
        )
