...
// ============================================================================
// ReverseEngineeringGroupWidget — a labeled tool group (tools on top, label below)
// ============================================================================

class ReverseEngineeringGui::ReverseEngineeringGroupWidget : public QWidget
{
    Q_OBJECT

public:
    explicit ReverseEngineeringGroupWidget(QWidget* parent = nullptr)
        : QWidget(parent)
    {
    ...
    {
        _menu = menu;
        connect(_labelButton, &QToolButton::clicked, this,
                &ReverseEngineeringGroupWidget::showMenu);
    }
...
// ============================================================================
// ReverseEngineeringPartToolsWidget
// ============================================================================

ReverseEngineeringPartToolsWidget::ReverseEngineeringPartToolsWidget(QWidget* parent)
    : QWidget(parent)
{
    ...
    _reconstructGroup = createReconstructGroup();
    _approximateGroup = createApproximateGroup();

    layout->addWidget(_reconstructGroup, 1);
    layout->addWidget(_approximateGroup, 1);
}

void ReverseEngineeringPartToolsWidget::resizeEvent(QResizeEvent* event)
{
    ...
    if (_reconstructGroup) {
        _reconstructGroup->updateVisibility(groupWidth);
    }
    if (_approximateGroup) {
        _approximateGroup->updateVisibility(groupWidth);
    }
}

ReverseEngineeringGroupWidget* ReverseEngineeringPartToolsWidget::createReconstructGroup()
{
    ...
    auto* group = createGroup(
        ...
        commands,
        buildReconstructMenu());

    return group;
}

ReverseEngineeringGroupWidget* ReverseEngineeringPartToolsWidget::createApproximateGroup()
{
    QVector<const char*> commands = {
        "Reen_ApproxSurface",
        "Reen_ApproxCurve",
    ...
    auto* group = createGroup(
        ...
        commands,
        buildApproximateMenu());

    return group;
}

ReverseEngineeringGroupWidget* ReverseEngineeringPartToolsWidget::createGroup(
    const QString& label,
    const QVector<const char*>& visibleCommands,
    QMenu* menu)
{
    auto* group = new ReverseEngineeringGroupWidget(this);
...
QToolButton* ReverseEngineeringPartToolsWidget::createToolButton(const char* cmdName)
{
...
QMenu* ReverseEngineeringPartToolsWidget::buildReconstructMenu()
{
    ...
    addSectionHeader(QObject::tr("RECONSTRUCTION"));
    addMenuItem("Reen_PoissonReconstruction");
    addMenuItem("Reen_ViewTriangulation");

    return menu;
}

QMenu* ReverseEngineeringPartToolsWidget::buildApproximateMenu()
{
    auto* menu = new QMenu(this);
    ...
    addMenuItem("Reen_ApproxSurface");

    addSeparator();

    // Curves section
    addSectionHeader(QObject::tr("CURVES"));
    addMenuItem("Reen_ApproxCurve");

    addSeparator();

    // Planes section
    addSectionHeader(QObject::tr("PLANES"));
    addMenuItem("Reen_ApproxPlane");

    return menu;
}

QIcon ReverseEngineeringPartToolsWidget::commandIcon(const char* cmdName)
{
    auto& cmdMgr = Gui::Application::Instance->commandManager();
    Gui::Command* cmd = cmdMgr.getCommandByName(cmdName);
    if (!cmd) {
...
#include "ReverseEngineeringPartToolsWidget.moc"
