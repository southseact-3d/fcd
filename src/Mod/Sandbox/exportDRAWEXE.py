...
def polygonstr(r, pcount):
    ...
    for i in range(pcount):
        ...
        v = m.multiply(v)
    points.append(v)
    return " ".join("%s %s %s" % (f2s(v.x), f2s(v.y), f2s(v.z)) for v in points)


def formatobjtype(ob):
    objtype = ob.TypeId
    if (
        ob.isDerivedFrom("Part::FeaturePython")
        or ob.isDerivedFrom("Part::Part2DObjectPython")
        or ob.isDerivedFrom("App::FeaturePython")
    ) and hasattr(ob.Proxy, "__module__"):
        return "%s::%s.%s" % (ob.TypeId, ob.Proxy.__module__, ob.Proxy.__class__.__name__)
    else:
        return ob.TypeId
...
def placement2draw(placement, name="object"):
    ...
    if not placement.Rotation.isNull():
        ...
        drawcommand += "trotate %s 0 0 0 %s %s %s %s\n" % (
            ...
            f2s(dx),
            f2s(dy),
            f2s(dz),  #            f2s(dx,axis=True),f2s(dy,axis=True),f2s(dz,axis=True),\
            f2s(an, angle=True),
        )
...
def isDraftFeature(ob):
    if (
        (ob.isDerivedFrom("Part::FeaturePython") or ob.isDerivedFrom("Part::Part2DObjectPython"))
        and hasattr(ob.Proxy, "__module__")
        and ob.Proxy.__module__ == "Draft"
    ):
        return True
    ...
    if isDraftFeature(ob):
        ...
        if isinstance(ob.Proxy, Draft._Wire):
            ...
            if (
                ob.Base is None
                and ob.Tool is None
                and ob.FilletRadius.Value == 0.0
                and ob.ChamferSize.Value == 0.0
            ):
                return True
...
def isOpenSCADFeature(ob):
    if (
        ob.isDerivedFrom("Part::FeaturePython")
        and hasattr(ob.Proxy, "__module__")
        and ob.Proxy.__module__ == "OpenSCADFeatures"
    ):
        return True
...
def isDeform(ob):
    ...
    return isOpenSCADMultMatrixFeature(ob) and ob.Matrix.analyze().startswith("Scale [")
...
class Drawexporter(object):
    ...
    def saveSweep(self, ob):
        ...
        if explodeshape:
            ...
            if len(subshapelst) and spine.Shape.ShapeType != "Edge":
                ...
                edgelst = " ".join(("%s_%s" % (spine.Name, ss[4:]) for ss in subshapelst))
            ...
            if len(subshapelst):
                path = Part.Wire(
                    [spine.Shape.getElement(subshapename) for subshapename in subshapelst]
                )
            elif spine.Shape.ShapeType == "Edge":
                path = spine.Shape
            elif spine.Shape.ShapeType == "Wire":
            ...
            spinename = "%s-0-spine" % ob.Name
            saveShape(
                self.csg, self.filename, path, spinename, None, self.cleanshape
        ...
        for i, subobj in enumerate(ob.Sections):
            ...
            addoptions = []
            explodeshape = self.alwaysexplode or self.process_object(subobj, True)
            if explodeshape:
                sh = subobj.Shape
                if (
                    sh.ShapeType == "Vertex"
                    or sh.ShapeType == "Wire"
                    or sh.ShapeType == "Edge"
                    or sh.ShapeType == "Face"
                    and len(sh.Wires) == 1
                ):
            ...
            if not explodeshape:  # extract only the used subshape
                ...
                saveShape(
                    self.csg, self.filename, sh, sectionname, None, self.cleanshape
                )  # placement with shape
    ...
    def process_object(self, ob, checksupported=False, toplevel=False):
        ...
        if ob.TypeId in ["Part::Cut", "Part::Fuse", "Part::Common", "Part::Section"]:
            ...
            d1.update(
                {
                    "uname": "%s-untrimmed" % d1["name"],
                    "length": f2s(ob.Length),
                    "width": f2s(ob.Width),
                }
            )
            self.csg.write("plane %s 0 0 0\n" % d1["uname"])
            self.csg.write("mkface %(name)s %(uname)s 0 %(length)s 0 %(width)s\n" % d1)
            ...
            d1.update(
                {
                    "uname": "%s-untrimmed" % d1["name"],
                    "maj": f2s(ob.MajorRadius),
                    "min": f2s(ob.MinorRadius),
            ...
            d1.update(
                {
                    "radius": f2s(ob.Radius),
                    "angle1": f2s(ob.Angle1),
                    "angle2": f2s(ob.Angle2),
                    "angle3": f2s(ob.Angle3),
                }
            )
            if ob.Angle1.Value == -90 and ob.Angle2.Value == 90 and ob.Angle3.Value == 360:
                self.csg.write("psphere %(name)s %(radius)s\n" % d1)
            else:
                self.csg.write(
                    "psphere %(name)s %(radius)s %(angle1)s " "%(angle2)s %(angle3)s\n" % d1
                )
            ...
            if checksupported:
                return True  # The object is supported
            d1.update({"radius": f2s(ob.Radius), "height": f2s(ob.Height), "angle": f2s(ob.Angle)})
            ...
            d1.update(
                {
                    "radius1": f2s(ob.Radius1),
                    "radius2": f2s(ob.Radius2),
                    "height": f2s(ob.Height),
                    "angle": f2s(ob.Angle),
                }
            )
            if ob.Angle.Value == 360:
                self.csg.write("pcone %(name)s %(radius1)s %(radius2)s " "%(height)s\n" % d1)
            else:
                self.csg.write(
                    "pcone %(name)s %(radius1)s %(radius2)s " "%(height)s %(angle)s\n" % d1
                )
            ...
            if ob.Angle1.Value == -180 and ob.Angle2.Value == 180 and ob.Angle3.Value == 360:
                self.csg.write("ptorus %(name)s %(radius1)s %(radius2)s\n" % d1)
            else:
                self.csg.write(
                    "ptorus %(name)s %(radius1)s %(radius2)s "
            ...
            if len(ob.Shapes) == 0:
            ...
            elif ob.TypeId == "Part::MultiFuse" and occversiontuple() >= (6, 8, 1):
                if checksupported:
                    return True  # The object is supported
                ...
                self.csg.write("baddtools %s\n" % " ".join(subobj.Name for subobj in ob.Shapes[1:]))
            ...
            (
                isDraftPolygon(ob)
                and ob.ChamferSize.Value == 0
                and ob.FilletRadius.Value == 0
                and ob.AttachmentSupport is None
            )
            or ob.TypeId == "Part::Prism"
            or ob.TypeId == "Part::RegularPolygon"
        ):
            if checksupported:
                return True  # The object is supported
            ...
            self.csg.write(
                "prism %s %s %s %s %s\n"
                % (d1["name"], facename, f2s(ob.Dir.x), f2s(ob.Dir.y), f2s(ob.Dir.z))
            )
            ...
            self.csg.write(
                ...
                % (
                    ...
                    ob.Base.Name,
                    " ".join(
                        ("%s %s" % (f2s(e[1]), "%s_%d" % (ob.Base.Name, e[0])) for e in ob.Edges)
                    ),
                )
            )
        elif ob.TypeId == "Part::Thickness" and not ob.SelfIntersection and ob.Mode == "Skin":
            if checksupported:
                return True  # The object is supported
            ...
            faces = " ".join([("%s_%s" % (baseobj.Name, f[4:])) for f in facelist])
            value = f2s(ob.Value)
            self.csg.write("explode %s F\n" % baseobj.Name)
            self.csg.write(
                "offsetparameter 1e-7 %s %s\n" % (inter[ob.Intersection], jointype[ob.Join])
            )
            ...
            sectionnames = []
            for i, subobj in enumerate(ob.Sections):
                explodeshape = self.alwaysexplode or self.process_object(suboobj, True)
            ...
            self.csg.write(
                "thrusections %s %d %d %s\n"
                % (ob.Name, int(ob.Solid), int(ob.Ruled), " ".join(sectionnames))
            )
        ...
        elif isDraftCircle(ob) or ob.TypeId == "Part::Circle" or isDraftEllipse(ob):
            if checksupported:
                return True  # The object is supported
            ...
            if ob.TypeId == "Part::Circle":
                ...
                makeface = ob.MakeFace and (ob.Shape.isNull() or ob.Shape.ShapeType == "Face")
            ...
            pointstr = " ".join("%s %s %s" % (f2s(v.x), f2s(v.y), f2s(v.z)) for v in points)
            self.csg.write("polyline %s %s\n" % (polyname, pointstr))
            if ob.MakeFace:
            ...
            if len(ob.Objects) == 1:
                ...
                for i, subobj in enumerate(ob.Objects):
                    ...
                    if x == y == z:  # uniform scaling
                        ...
                        self.csg.write(
                            "deform %(newname)s %(basename)s" " %(cx)s %(cy)s %(cz)s\n" % d1
                        )
                self.csg.write("compound %s %s\n" % (" ".join(newnames), ob.Name))
        elif (
            isDraftShape2DView(ob)
            and not ob.Tessellation
            and ob.ProjectionMode == "Solid"
            and ob.Base is not None
            and hasattr(ob.Base, "Shape")
        ):
            ...
            x = ax2_xdir(v)
            self.csg.write(
                "hprj %s_proj 0 0 0 %s %s %s %s %s %s\n"
                % (ob.Name, f2s(v.x), f2s(v.y), f2s(v.z), f2s(x.x), f2s(x.y), f2s(x.z))
            )
            ...
            hasplacement = saveShape(
                self.csg, self.filename, ob.Shape, ob.Name, hasplacement, self.cleanshape
            )
            ...
            if ob.isDerivedFrom("App::Annotation"):
                ...
                self.csg.write(
                    ...
                    % (
                        ...
                        f2s(v.y),
                        f2s(v.z),
                        "\\n".join(ob.LabelText).encode("ascii", errors="xmlcharrefreplace"),
                    )
                )

    def export_objects(self, objlst, toplevel=True):
        self.write_header()
        toplevelobjs = [self.process_object(ob, toplevel=toplevel) for ob in objlst]
