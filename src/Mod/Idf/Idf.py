...
IDF_sort = (
    0  # 0-sort per refdes [1 - part number (not preferred)/refdes] 2-sort per footprint/refdes
)
...
def process_emn(doc, filename):
    ...
    for emnline in emnlines:
        ...
        if (
            current_section == ".PLACEMENT"
            and section_counter > 1
            and fmod(section_counter, 2) == 0
        ):
            place_item = []
    ...
    FreeCAD.Console.PrintMessage(
        "Proceed "
        + str(Process_board_outline(doc, board_outline, drills, board_thickness))
        + " outlines\n"
    )
    ...
    for point in board_outline:
        ...
        if vertex_index == 0:
            ...
            if point[3] != 0 and point[3] != 360:
                out_shape.append(
                    Part.Arc(prev_vertex, mid_point(prev_vertex, vertex, point[3]), vertex)
                )
                FreeCAD.Console.PrintMessage("mid point " + str(mid_point) + "\n")
            elif point[3] == 360:
                per_point = Per_point(prev_vertex, vertex)
                out_shape.append(
                    Part.Arc(per_point, mid_point(per_point, vertex, point[3] / 2), vertex)
                )
                out_shape.append(
                    Part.Arc(per_point, mid_point(per_point, vertex, -point[3] / 2), vertex)
                )
...
def mid_point(prev_vertex, vertex, angle):
    ...
    shift = (
        (1 - cos(angle))
        * hypot(vertex.y - prev_vertex.y, vertex.x - prev_vertex.x)
        / 2
        / sin(angle)
    )
    midpoint = Base.Vector(
...
def process_emp(doc, filename, placement, board_thickness):
    ...
    for empline in emplines:
        ...
        if len(emprecords[0]) > 4 and emprecords[0][0:4] == ".END":
            ...
            if comp_PartNumber != "":
                ...
                comps.append(
                    (
                        comp_PartNumber,
                        [Process_comp_outline(doc, comp_outline, comp_height), comp_GeometryName],
                    )
                )
                comp_PartNumber = ""
                comp_outline = []
        ...
        if (
            (current_section == ".ELECTRICAL" or current_section == ".MECHANICAL")
            and section_counter == 2
            and emprecords[2] == "THOU"
        ):
            emp_unit = 0.0254
        if (
            (current_section == ".ELECTRICAL" or current_section == ".MECHANICAL")
    ...
    for point in comp_outline:
        ...
        if vertex_index > 0:
            if point[2] != 0 and point[2] != 360:
                out_shape.append(
                    Part.Arc(prev_vertex, mid_point(prev_vertex, vertex, point[2]), vertex)
                )
                FreeCAD.Console.PrintMessage("mid point " + str(mid_point) + "\n")
            elif point[2] == 360:
                per_point = Per_point(prev_vertex, vertex)
                out_shape.append(
                    Part.Arc(per_point, mid_point(per_point, vertex, point[2] / 2), vertex)
                )
                out_shape.append(
                    Part.Arc(per_point, mid_point(per_point, vertex, -point[2] / 2), vertex)
                )
...
def place_steps(doc, placement, board_thickness):
    ...
    for model_line in model_lines:
        ...
        if len(model_records) > 1 and model_records[0] and not model_records[0] in keys:
            ...
            model_dict.append(
                (str(model_records[0]).replace('"', ""), str(model_records[1]).replace('"', ""))
            )
    model_dict = dict(model_dict)
    validkeys = filter(
        lambda x: x in [place_item[2] for place_item in placement], model_dict.keys()
    )
    FreeCAD.Console.PrintMessage(
        "Step models to be loaded for footprints: " + str(validkeys) + "\n"
    )
...
):  # rotation heading=around Y, attitude =around Z,  bank attitude =around X
