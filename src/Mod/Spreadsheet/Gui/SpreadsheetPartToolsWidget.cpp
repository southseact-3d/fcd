...
// ============================================================================
// SpreadsheetGroupWidget — a labeled tool group (tools on top, label below)
// ============================================================================

class SpreadsheetGui::SpreadsheetGroupWidget : public QWidget
{
    Q_OBJECT

public:
    explicit SpreadsheetGroupWidget(QWidget* parent = nullptr)
        : QWidget(parent)
    {
    ...
    {
        _menu = menu;
        connect(_labelButton, &QToolButton::clicked, this, &SpreadsheetGroupWidget::showMenu);
    }
...
// ============================================================================
// SpreadsheetPartToolsWidget
// ============================================================================

SpreadsheetPartToolsWidget::SpreadsheetPartToolsWidget(QWidget* parent)
    : QWidget(parent)
{
    ...
    _sheetGroup = createSheetGroup();
    _formatGroup = createFormatGroup();
    _styleGroup = createStyleGroup();

    layout->addWidget(_sheetGroup, 1);
    layout->addWidget(_formatGroup, 1);
    layout->addWidget(_styleGroup, 1);
}

void SpreadsheetPartToolsWidget::resizeEvent(QResizeEvent* event)
{
    ...
    if (_sheetGroup) {
        _sheetGroup->updateVisibility(groupWidth);
    }
    if (_formatGroup) {
        _formatGroup->updateVisibility(groupWidth);
    }
    if (_styleGroup) {
        _styleGroup->updateVisibility(groupWidth);
    }
}

SpreadsheetGroupWidget* SpreadsheetPartToolsWidget::createSheetGroup()
{
...
SpreadsheetGroupWidget* SpreadsheetPartToolsWidget::createFormatGroup()
{
    QVector<const char*> commands = {
...
SpreadsheetGroupWidget* SpreadsheetPartToolsWidget::createStyleGroup()
{
    QVector<const char*> commands = {
...
SpreadsheetGroupWidget* SpreadsheetPartToolsWidget::createGroup(
    const QString& label,
    const QVector<const char*>& visibleCommands,
    QMenu* menu)
{
    auto* group = new SpreadsheetGroupWidget(this);
...
QToolButton* SpreadsheetPartToolsWidget::createToolButton(const char* cmdName)
{
...
QMenu* SpreadsheetPartToolsWidget::buildSheetMenu()
{
    ...
    // Sheet section
    addSectionHeader(QObject::tr("SHEET"));
    addMenuItem("Spreadsheet_CreateSheet");

    addSeparator();

    // Import/Export section
    addSectionHeader(QObject::tr("IMPORT/EXPORT"));
    addMenuItem("Spreadsheet_Import");
    addMenuItem("Spreadsheet_Export");

    return menu;
}

QMenu* SpreadsheetPartToolsWidget::buildFormatMenu()
{
    auto* menu = new QMenu(this);
    ...
    // Cells section
    addSectionHeader(QObject::tr("CELLS"));
    addMenuItem("Spreadsheet_MergeCells");
    addMenuItem("Spreadsheet_SplitCell");

    addSeparator();

    // Alignment section
    addSectionHeader(QObject::tr("ALIGNMENT"));
    addMenuItem("Spreadsheet_AlignLeft");
    addMenuItem("Spreadsheet_AlignCenter");
    addMenuItem("Spreadsheet_AlignRight");

    return menu;
}

QMenu* SpreadsheetPartToolsWidget::buildStyleMenu()
{
    auto* menu = new QMenu(this);
    ...
    // Text Style section
    addSectionHeader(QObject::tr("TEXT STYLE"));
    addMenuItem("Spreadsheet_StyleBold");
    addMenuItem("Spreadsheet_StyleItalic");
    addMenuItem("Spreadsheet_StyleUnderline");

    addSeparator();

    // Properties section
    addSectionHeader(QObject::tr("PROPERTIES"));
    addMenuItem("Spreadsheet_SetAlias");

    return menu;
}

QIcon SpreadsheetPartToolsWidget::commandIcon(const char* cmdName)
{
    auto& cmdMgr = Gui::Application::Instance->commandManager();
    Gui::Command* cmd = cmdMgr.getCommandByName(cmdName);
    if (!cmd) {
...
#include "SpreadsheetPartToolsWidget.moc"
