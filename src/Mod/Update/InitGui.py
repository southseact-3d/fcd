...
# Resolve icon at import time; fall back to empty string if missing.
_UPDATE_ICON = os.path.join(os.path.dirname(__file__), "icon.svg")
if not os.path.isfile(_UPDATE_ICON):
    _UPDATE_ICON = ""
        ...
        def do_check():
            try:
                ...
                FreeCAD.Console.PrintError(
                    "Check for Updates: error during update check\n"
                    + traceback.format_exc()
                )
                result = {
                    "available": False,
                    "message": "Update check failed (see console for details).",
                }
    ...
    def _apply_patch(self, mw):
        ...
        def do_apply():
            try:
                ...
                FreeCAD.Console.PrintError(
                    "Check for Updates: error during patch application\n"
                    + traceback.format_exc()
                )
                success = False
                message = "Update failed (see console for details)."
            QtCore.QMetaObject.invokeMethod(
                _main2, "onApplyDone",
                QtCore.Qt.QueuedConnection,
