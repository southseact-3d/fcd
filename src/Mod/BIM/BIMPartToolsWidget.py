...
"""BIM Part Tools Widget — Python port of the PartDesign PartToolsWidget pattern.

Provides a compact toolbar widget with grouped tool buttons for the BIM workbench.
Groups: Drafting, 3D/BIM, Modify. Each group shows icon-only buttons with a
...
class BIMPartToolsWidget(QtWidgets.QWidget):
    """Compact BIM toolbar widget with Drafting / 3D-BIM / Modify groups."""
    ...
    def __init__(self, parent=None):
        ...
        self._draftGroup = self._createDraftingGroup()
        self._bimGroup = self._create3DBIMGroup()
        self._modGroup = self._createModifyGroup()

        layout.addWidget(self._draftGroup, 1)
        layout.addWidget(self._bimGroup, 1)
        layout.addWidget(self._modGroup, 1)

    # -- group factories ----------------------------------------------------

    def _createDraftingGroup(self):
        commands = ["BIM_Sketch", "Draft_Line", "Draft_Wire", "Draft_Rectangle", "Draft_Circle"]
        menu = self._buildMenu(commands)
        return self._buildGroup("Drafting", commands, menu)

    def _create3DBIMGroup(self):
        commands = ["Arch_Wall", "BIM_Column", "BIM_Beam", "BIM_Slab", "BIM_Door"]
        menu = self._buildMenu(commands)
        return self._buildGroup("3D/BIM", commands, menu)

    def _createModifyGroup(self):
        commands = ["Draft_Move", "Draft_Rotate", "Draft_Mirror", "BIM_Extrude"]
        menu = self._buildMenu(commands)
    ...
    def resizeEvent(self, event):
        ...
        self._draftGroup.updateVisibility(groupWidth)
        self._bimGroup.updateVisibility(groupWidth)
