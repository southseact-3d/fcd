...
def p_group_action1(p):
    ...
    if len(p[5]) > 1:
        ...
        if printverbose:
            print(f"Group {p[5]} type {type(p[5])}")
        checkObjShape(p[5])
        p[0] = p[5]
...
def p_not_supported(p):
    ...
    """
    if gui and not FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").GetBool(
        ...
        QtGui.QMessageBox.critical(
            None,
            translate("OpenSCAD", "Unsupported Function") + " : " + p[1],
...
def p_rotate_extrude_action(p):
    ...
    fnmax = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").GetInt(
        "useMaxFN", 16
    )
    if len(p[6]) > 1:
...
def p_multmatrix_action(p):
    ...
    elif FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").GetBool(
...
def p_projection_action(p):
    ...
    doc.recompute()
    p[6][0].Shape.tessellate(
        0.05
    )  # Ensure the bounding box calculation is not done with the splines, which can give a bad result
    ...
    plane.Placement = FreeCAD.Placement(FreeCAD.Vector(bbox.XMin, bbox.YMin, 0), FreeCAD.Rotation())
    if gui:
