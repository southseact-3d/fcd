...
class Node:
    ...
    fnmin = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").GetInt("useMaxFN")
    ...
    def addtofreecad(self, doc=None, fcpar=None):
        ...
        if namel in multifeature:
            ...
            if len(self.children) == 1:
                ...
                if len(self.children) == 2:
                    ...
                    tool = Node(name="imp_union", children=self.children[1:]).addtofreecad(doc, obj)
                obj.Base = base
                obj.Tool = tool
            ...
            if (
                "$fn" in self.arguments
                and self.arguments["$fn"] > 2
            ...
            shell = Part.Shell(
                [
                    Part.Face(
                        Part.makePolygon(
                            [tuple(points[pointindex]) for pointindex in (face + face[0:1])]
                        )
                    )
                    for face in faces
                ]
                    ...
                    Part.Face(
                        Part.makePolygon(
                            [
                                ...
                                for pointindex in (path + path[0:1])
                            ]
                        )
                    )
                    for path in paths
            ...
            if len(self.children) == 1:
                ...
                obj = Node(name="imp_union", children=self.children).addtofreecad(
                    doc, fcpar or True
                )
            ...
            if isspecialorthogonalpython(
                fcsubmatrix(m1)
            ):  # a Placement can represent the transformation
                if len(self.children) == 1:
                    obj = self.children[0].addtofreecad(doc, fcpar or True)
                else:
                    obj = Node(name="imp_union", children=self.children).addtofreecad(
                ...
                if len(self.children) == 1:
                    ...
                    child = Node(name="imp_union", children=self.children).addtofreecad(doc, obj)
        ...
        # elif namel == 'import': pass #Custom Feature
            ...
            if len(self.children) == 0:
                ...
                base = Node(name="imp_union", children=self.children).addtofreecad(doc, obj)
            ...
            if self.arguments["cut"]:
                ...
                if not plane:
                    ...
                    plane.Placement = FreeCAD.Placement(
                        FreeCAD.Vector(-Node.planedim, -Node.planedim, 0),
            ...
            if filename:
                ...
                if extension.lower() in reverseimporttypes()["Mesh"]:
                    ...
                    solid = Part.Solid(sh)
                    obj = doc.addObject(
                        "Part::FeaturePython", "import_%s_%s" % (extension, objname)
                    )
                    ImportObject(obj, mesh1)  # This object is not mutable from the GUI
                    ViewProviderTree(obj.ViewObject)
                    solid = solid.removeSplitter()
                    ...
                    obj = doc.addObject(
                        "Part::FeaturePython", "import_dxf_%s_%s" % (objname, layera)
                    )
                ...
                if obj:  # handle origin and scale
                    if scale is not None and scale != 1:
                        ...
                        placement = FreeCAD.Placement(
                            FreeCAD.Vector(*[-c for c in origin]), FreeCAD.Rotation()
                        )
            ...
            if (
                len(self.children) == 2
                and childrennames.count("cube") == 1
                and (childrennames.count("sphere") + childrennames.count("cylinder")) == 1
            ):
                ...
                r = roundobj.arguments.get("r") or roundobj.arguments.get("r1")
                ...
                nonsphere = self.children[0:sphereindex] + self.sphere[sphereindex + 1 :]
                obj = doc.addObject("Part::FeaturePython", "Offset")
                if len(nonsphere) == 1:
                    child = nonsphere[0].addtofreecad(doc, obj)
                else:
                    child = Node(name="imp_union", children=nonsphere).addtofreecad(doc, obj)
        ...
        if fcpar:
            ...
            if True:  # never refine the Shape, as it itroduces crashes
                ...
                if (
                    obj.Type == "Part::Extrusion"
                    and obj.Base.Type == "Part::Part2DObjectPython"
                    and isinstance(obj.Base.Proxy, Draft._Polygon)
                    or (
                        not obj.isDerivedFrom("Part::Extrusion")
                        and not obj.isDerivedFrom("Part::Boolean")
                        and not obj.isDerivedFrom("Part::Cut")
                        and not obj.isDerivedFrom("Part::MultiCommon")
                        and not obj.isDerivedFrom("Part::MultiFuse")
                        and not obj.isDerivedFrom("Part::Revolution")
                    )
                    or (
                        obj.isDerivedFrom("Part::FeaturePython")
                        and isinstance(obj.Proxy, RefineShape)
                    )
                ):
                    return obj
                else:
    ...
    def flattengroups(self, name="group"):
        ...
        node.children = [
            ...
            if not (len(child.children) == 0 and child.name == name)
        ]
        if node.children:
