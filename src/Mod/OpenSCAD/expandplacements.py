...
def likeprimitive(obj, extrusion=False):
    ...
    return (
        not obj.OutList
        or obj.isDerivedFrom("Part::Extrusion")
        or extrusion
        and (obj.isDerivedFrom("Part::Revolution") or obj.isDerivedFrom("Part::FeaturePython"))
        or not obj.isDerivedFrom("Part::Feature")
    )
...
def expandplacementsmatrix(obj, matrix):
    ...
    if obj.isDerivedFrom("Part::Feature") and isinstance(obj.Proxy, MatrixTransform):
        ...
        if isspecialorthogonal(ownmatrix):
            ...
            for parent in obj.InList:
                replaceobj.replaceobj(
                    parent, obj, newobj
                )  # register the new object in the feature tree
            obj.Placement = FreeCAD.Placement()
    else:  # not a primitive
        for outobj in obj.OutList:
            if outobj.isDerivedFrom("Part::Feature") and isinstance(obj.Proxy, MatrixTransform):
