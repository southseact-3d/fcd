...
class Overlappingfaces:
    ...
    def finddepth(dict1, faceidx, curdepth=0):
        if faceidx not in dict1:
            ...
            return max(
                [
                    (Overlappingfaces.finddepth(dict1, childface, curdepth + 1))
                    for childface in dict1[faceidx]
                ]
            )
    ...
    def findrootdepth(self):
        return max(
            [
                Overlappingfaces.finddepth(self.isinsidedict, fi)
                for fi in range(len(self.sortedfaces))
            ]
        )
    ...
    def printtree(isinsidedict, facenum):
        ...
        rootitems = [
            fi for fi in range(facenum) if Overlappingfaces.hasnoparentstatic(isinsidedict, fi)
        ]
    ...
    def makeshape(self):
        ...
        finishedwith = []
        while not all(
            [Overlappingfaces.hasnoparentstatic(isinsidedict, fi) for fi in range(len(faces))]
        ):
...
def findConnectedEdges(edgelist, eps=1e-6, debug=False):
    ...
    while freeedges:
        ...
        for forward in (True, False):
            found = True
            while found:
                lastvert = newedge[vertindex(forward)][0].Vertexes[
                    vertindex(forward == newedge[vertindex(forward)][1])
                ]
                for ceindex, checkedge in enumerate(freeedges):
                    found = False
                    for cvindex, cvert in enumerate(
                        [checkedge.Vertexes[0], checkedge.Vertexes[-1]]
                    ):
...
def endpointdistance(edges):
    ...
    if numedges > 1:
        innerdistances = [
            edges[i].Vertexes[-1].Point.sub(edges[i + 1].Vertexes[0].Point).Length
    ...
    for i in range(len(debuglist)):
        ...
        if prev:
            if curr[0].Vertexes[-1 * (not curr[1])].Point == prev[0].Vertexes[-1 * prev[1]].Point:
                ...
                p1 = median(
                    curr[0].Vertexes[-1 * (not curr[1])].Point, prev[0].Vertexes[-1 * prev[1]].Point
                )
        else:
            p1 = curr[0].Vertexes[-1 * (not curr[1])].Point
        if nexte:
