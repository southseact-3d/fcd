...
class ExplodeGroup:
    ...
    def Activated(self):
        def isdefault(shapecolor):
            ...
            scol = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/View").GetUnsigned(
            ...
            return all(
                all(comparefloat(fcc, dcc) for fcc, dcc in zip(facecolor, defaultcolor))
                for facecolor in shapecolor
            )

        def isgrey(shapecolor):
            defaultcolor = (
                float.fromhex("0x1.99999ap-1"),
                float.fromhex("0x1.99999ap-1"),
                float.fromhex("0x1.99999ap-1"),
                0.0,
            )
        ...
        def explode(obj, color=True):
            if (
                obj.isDerivedFrom("Part::Fuse")
                or obj.isDerivedFrom("Part::MultiFuse")
                or obj.isDerivedFrom("Part::Compound")
            ):
                plm = obj.Placement
                outlist = obj.OutList[:]
                if plm.isNull() or all(
                    (len(oo.InList) == 1 and not oo.isDerivedFrom("PartDesign::Feature"))
                    ...
                    FreeCAD.Console.PrintError(
                        translate("OpenSCAD", "Unable to explode %s") % obj.Name + "\n"
                    )
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_Explode_Group",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("OpenSCAD_ExplodeGroup", "Explode Group"),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_ExplodeGroup", "Explodes a fusion or compound and applies random colors"
            ),
        }
...
class ColorCodeShape:
    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_ColorCodeShape", "Colors shapes by validity and type"
            ),
        }
...
class RefineShapeFeature:
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_RefineShapeFeature",
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_RefineShapeFeature", "Refine Shape Feature"
            ),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_RefineShapeFeature", "Creates a refined shape"
            ),
        }
...
class MirrorMeshFeature:
    ...
    def Activated(self):
        ...
        for selobj in selection:
            ...
            item, ok = QtGui.QInputDialog.getItem(
                ...
                "Mirror about which Axis?",
                "Select Axis (or enter custom value):",
                items,
                editable=True,
            )
            if ok:
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_MirrorMeshFeature",
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
...
class ScaleMeshFeature:
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_ScaleMeshFeature",
...
class ResizeMeshFeature:
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_ResizeMeshFeature",
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
...
class IncreaseToleranceFeature:
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_IncreaseToleranceFeature",
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_IncreaseToleranceFeature", "Increase Tolerance Feature"
            ),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_IncreaseToleranceFeature", "Creates a feature to increase the tolerance"
            ),
        }
...
class ExpandPlacements:
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_ExpandPlacements",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("OpenSCAD_ExpandPlacements", "Expand Placements"),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_ExpandPlacements", "Expands all placements downwards in the Tree View"
            ),
        }
...
class ReplaceObject:
    def IsActive(self):
        ...
        if nobj == 3:
            ...
            return tuple((len(obj.InList)) for obj in FreeCADGui.Selection.getSelection()) in (
                (0, 1),
                (1, 0),
            )
        # else: return False
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_ReplaceObject",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("OpenSCAD_ReplaceObject", "Replace Object"),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_ReplaceObject", "Replaces an object in the Tree View"
            ),
        }
...
class RemoveSubtree:
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_RemoveSubtree",
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_RemoveSubtree", "Remove Objects and Children"
            ),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_RemoveSubtree",
                "Removes the selected objects and all children that are not referenced by other objects",
            ),
        }
    ...
    def __init__(self, *args):
        ...
        def undoable_clear():
            ...
            keypress = QtGui.QKeyEvent(
                QtGui.QKeyEvent.KeyPress, QtCore.Qt.Key_Delete, QtCore.Qt.NoModifier
            )
...
class OpenSCADMeshBooleanTask:
    ...
    def doboolean(self):
        ...
        if self.form.rb_intersection.isChecked():
            opname = "intersection"
        elif self.form.rb_difference.isChecked():
            opname = "difference"
        elif self.form.rb_hull.isChecked():
            opname = "hull"
        elif self.form.rb_minkowski.isChecked():
            opname = "minkowski"
        else:
            opname = "union"
        newmesh, objsused = meshoponobjs(opname, FreeCADGui.Selection.getSelection())
        if len(objsused) > 0:
            newmeshobj = FreeCAD.activeDocument().addObject(
                "Mesh::Feature", opname
            )  # create a Feature for the result
...
class AddOpenSCADElement:
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_AddOpenSCADElement",
            "MenuText": QtCore.QT_TRANSLATE_NOOP(
...
class OpenSCADMeshBoolean:
    def IsActive(self):
        return (
            not FreeCADGui.Control.activeDialog() and len(FreeCADGui.Selection.getSelection()) >= 1
        )
    ...
    def GetResources(self):
        return {
            "Pixmap": "OpenSCAD_MeshBooleans",
            "MenuText": QtCore.QT_TRANSLATE_NOOP("OpenSCAD_MeshBoolean", "Mesh Boolean"),
            "ToolTip": QtCore.QT_TRANSLATE_NOOP(
                "OpenSCAD_MeshBoolean", "Performs a boolean operation using the OpenSCAD binary"
            ),
        }
