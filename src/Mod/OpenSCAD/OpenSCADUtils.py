...
def getopenscadexe(osfilename=None):
    if not osfilename:
        osfilename = FreeCAD.ParamGet("User parameter:BaseApp/Preferences/Mod/OpenSCAD").GetString(
            "openscadexecutable"
        )
...
def searchforopenscadexe():
    ...
    if sys.platform == "win32":
        testpaths = [
            os.path.join(os.environ.get("Programfiles(x86)", "C:"), "OpenSCAD\\openscad.exe")
        ]
        if "ProgramW6432" in os.environ:
            testpaths.append(
                os.path.join(os.environ.get("ProgramW6432", "C:"), "OpenSCAD\\openscad.exe")
            )
        ...
        if not outputfilename:
            ...
            if keepname:
                outputfilename = os.path.join(
                    dir1, "%s.%s" % (os.path.split(inputfilename)[1].rsplit(".", 1)[0], outputext)
                )
            else:
                outputfilename = os.path.join(dir1, "%s.%s" % (next(tempfilenamegen), outputext))
...
def call_openscad_with_pipes(input_filename, output_filename, output_extension, keep_name):
    ...
    with open(input_filename) as datafile:
        ...
        p = subprocess.Popen(
            [getopenscadexe(), "--export-format", "csg", "-o", "-", "-"],
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
        ...
        if not output_filename:
            ...
            if keep_name:
                output_filename = os.path.join(
                    dir1,
                    "%s.%s"
                    % (os.path.split(input_filename)[1].rsplit(".", 1)[0], output_extension),
                )
            else:
                output_filename = os.path.join(
                    dir1, "%s.%s" % (next(tempfilenamegen), output_extension)
                )
...
def isorthogonal(submatrix, precision=4):
    ...
    return [[round(f, precision) for f in line] for line in prod] == [
        [1, 0, 0],
        [0, 1, 0],
        [0, 0, 1],
    ]
...
def isspecialorthogonal(mat, precision=4):
    return abs(mat.submatrix(3).isOrthogonal(10 ** (-precision)) - 1.0) < 10 ** (
        -precision
    ) and abs(mat.submatrix(3).determinant() - 1.0) < 10 ** (-precision)
...
def meshopinline(opname, iterable1):
    ...
    """
    return callopenscadmeshstring(
        "%s(){%s}" % (opname, " ".join((mesh2polyhedron(meshobj) for meshobj in iterable1)))
    )
...
def meshoptempfile(opname, iterable1):
    ...
    meshimports = " ".join(
        'import(file = "%s");' % os.path.split(filename)[1] for filename in filenames  # filename \
    )
    ...
    for obj in inobjs:
        if obj.isDerivedFrom("Mesh::Feature"):
            ...
            if False:  # disabled due to issue 1292
                ...
                meshes.append(
                    Mesh.Mesh(obj.Shape.tessellate(params.GetFloat("meshmaxlength", 1.0)))
                )
        else:
...
def process2D_ObjectsViaOpenSCADShape(ObjList, Operation, doc):
    ...
    # https://www.freecad.org/tracker/view.php?id=3419
    dxfimports = " ".join(
        'import(file = "%s" %s);' % (os.path.split(filename)[1], fnStr)  # filename \
        for filename in filenames
    )
    #
...
def process_ObjectsViaOpenSCADShape(doc, children, name, maxmeshpoints=None):
    if all((not obj.Shape.isNull() and obj.Shape.Volume == 0) for obj in children):
        return process2D_ObjectsViaOpenSCADShape(children, name, doc)
    elif all((not obj.Shape.isNull() and obj.Shape.Volume > 0) for obj in children):
...
def process_ObjectsViaOpenSCAD(doc, children, name):
    if all((not obj.Shape.isNull() and obj.Shape.Volume == 0) for obj in children):
        ...
        FreeCAD.Console.PrintError(
            translate("OpenSCAD", "Error: either all shapes must be 2D or all shapes must be 3D")
            + "\n"
        )
