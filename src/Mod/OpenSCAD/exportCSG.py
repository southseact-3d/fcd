...
def check_multmatrix(csg, ob, x, y, z):
    ...
    if ob.Placement.isNull():
    ...
    elif ob.Placement.Rotation.isNull() and (ob.Placement.Base - b).Length < 1e-6:
        ...
        csg.write("multmatrix([[" + str(m.A11) + ", " + str(m.A12) + ", " + str(m.A13) + ",\
            " + str(m.A14) + "], [" + str(m.A21) + ", " + str(m.A22) + ", " + str(m.A23) + ", " + str(m.A24) + "], [" + str(m.A31) + ", " + str(m.A32) + ", " + str(m.A33) + ", " + str(m.A34) + "], [\
             0, 0, 0, 1]]){\n")
...
def shape2polyhedron(shape):
    ...
    return mesh2polyhedron(
        MeshPart.meshFromShape(Shape=shape, Deflection=params.GetFloat("meshdeflection", 0.0))
    )
...
def process_object(csg, ob):
    ...
    if ob.TypeId == "Part::Sphere":
        ...
        csg.write(
            ...
            + str(ob.Height.Value)
            + ", r1 = "
            + str(ob.Radius1.Value)
            + ", r2 = "
            + str(ob.Radius2.Value)
            + ", center = "
            + center(mm)
            + ");\n"
        )
        ...
        print(ob.Base.Name)
        if (
            ob.Base.isDerivedFrom("Part::Part2DObjectPython")
            and hasattr(ob.Base, "Proxy")
            and hasattr(ob.Base.Proxy, "TypeId")
        ):
            ...
            csg.write(
                "square (size = ["
                + str(ob.Base.Length.Value)
                + ", "
                + str(ob.Base.Width.Value)
                + "], center = "
                + center(mm)
                + ");\n}\n"
            )
