...
"""
    ...
    for t in TREE_TYPES:
        ...
        TMPL_SEPARATOR.append(
            TMPL_SEPARATOR_DEF % (",".join([coord_t for _ in range(dim)]), data_t)
        )

        TMPL_RECORD.append(
            TMPL_RECORD_DEF % (dim, coord_t_short, data_t_short, dim, coord_t, data_t)
        )

        TMPL_IN_CONV_RECORD.append(
            TMPL_IN_CONV_RECORD_DEF
            % (
                dim,
                coord_t_short,
                ...
                py_coord_t * dim,
                py_data_t,
                ",".join(["&temp.point[%i]" % i for i in range(dim)]),
                dim,
                dim,
                coord_t,
                data_t,
        ...
        TMPL_IN_CONV_POINT.append(
            ...
            % (
                ...
                data_t_short,
                py_coord_t * dim,
                ",".join(["&point[%i]" % i for i in range(dim)]),
                dim,
            )
        )

        TMPL_OUT_CONV_RECORD.append(
            TMPL_OUT_CONV_RECORD_DEF
            % (
                dim,
                coord_t_short,
                ...
                py_coord_t * dim,
                py_data_t,
                ",".join(["(*iter).point[%i]" % i for i in range(dim)]),
            )
        )
        TMPL_OUT_CONV_POINT.append(
            TMPL_OUT_CONV_POINT_DEF
            % (
                dim,
                coord_t_short,
                ...
                data_t_short,
                py_coord_t * dim,
                ",".join(["r->point[%i]" % i for i in range(dim)]),
                py_data_t,
            )
        )

        TMPL_PY_CLASS.append(
            TMPL_PY_CLASS_DEF
            % (dim, coord_t_short, data_t_short, dim, coord_t.capitalize(), dim, coord_t, data_t)
        )

    TMPL_BODY_LIST = []
    for i in range(len(TREE_TYPES)):
        TMPL_BODY_LIST.append(
            TMPL_SEPARATOR[i]
            + "\n"
            + TMPL_RECORD[i]
            + "\n"
    ...
    i_content = i_content.replace("%%TMPL_BODY%%", TMPL_BODY).replace(
        "%%TMPL_PY_CLASS_DEF%%", "\n".join(TMPL_PY_CLASS)
    )
