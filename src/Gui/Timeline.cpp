...
void TimelineWidgetItem::setCurrent(bool current)
{
    m_isCurrent = current;
    if (current) {
        setStyleSheet(QStringLiteral("QToolButton { background-color: #3498db; border: 2px solid #2980b9; border-radius: 4px; }"));
    } else {
        setStyleSheet(QStringLiteral("QToolButton { background-color: transparent; border: 1px solid #555; border-radius: 4px; } QToolButton:hover { background-color: #444; }"));
    }
}
...
void Timeline::loadOperationIcons()
{
    ...
    m_operationIcons["LinearPattern"] = QIcon::fromTheme(QStringLiteral("PartDesign_LinearPattern"));
    m_operationIcons["PolarPattern"] = QIcon::fromTheme(QStringLiteral("PartDesign_PolarPattern"));
    m_operationIcons["MultiTransform"] = QIcon::fromTheme(QStringLiteral("PartDesign_MultiTransform"));
...
void Timeline::updateOperationsList()
{
    ...
    for (auto* obj : objects) {
        ...
        connect(widget, &TimelineWidgetItem::editRequested, this, &Timeline::onOperationEdited);
        connect(widget, &TimelineWidgetItem::deleteRequested, this, &Timeline::onOperationDeleted);
...
void Timeline::updateSliderRange()
{
    ...
    m_positionLabel->setText(QString::number(m_currentPosition + 1) + QStringLiteral(" / ") + QString::number(m_operations.size()));
}
...
void Timeline::setCurrentPosition(int position)
{
    ...
    m_positionLabel->setText(QString::number(position + 1) + QStringLiteral(" / ") + QString::number(m_operations.size()));
...
{
    if (!m_document || position < 0 || position >= static_cast<int>(m_operations.size())) {
        return;
    }
}
