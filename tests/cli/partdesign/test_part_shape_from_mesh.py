"""CLI test for PartDesign_PartShapeFromMesh command.

Bridge to Part command (dialog: Shape from mesh)
...
COMMAND_ID = "PartDesign_PartShapeFromMesh"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartShapeFromMesh")
    if cmd is None:
        errors.append("command 'PartDesign_PartShapeFromMesh' is not registered in the command manager")
    else:
