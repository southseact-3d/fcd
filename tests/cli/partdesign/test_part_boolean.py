"""CLI test for PartDesign_PartBoolean command.

Bridge to Part command (dialog: Boolean dialog)
...
COMMAND_ID = "PartDesign_PartBoolean"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartBoolean")
    if cmd is None:
        errors.append("command 'PartDesign_PartBoolean' is not registered in the command manager")
    else:
