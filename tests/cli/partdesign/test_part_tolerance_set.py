"""CLI test for PartDesign_PartToleranceSet command.

Bridge to Part command (dialog: Tolerance set)
...
COMMAND_ID = "PartDesign_PartToleranceSet"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartToleranceSet")
    if cmd is None:
        errors.append("command 'PartDesign_PartToleranceSet' is not registered in the command manager")
    else:
