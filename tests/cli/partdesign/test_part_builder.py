"""CLI test for PartDesign_PartBuilder command.

Bridge to Part command (dialog: Shape builder dialog (no object))
...
COMMAND_ID = "PartDesign_PartBuilder"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartBuilder")
    if cmd is None:
        errors.append("command 'PartDesign_PartBuilder' is not registered in the command manager")
    else:
