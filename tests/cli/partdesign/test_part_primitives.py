"""CLI test for PartDesign_PartPrimitives command.

Bridge to Part command (dialog: Primitives dialog (no object))
...
COMMAND_ID = "PartDesign_PartPrimitives"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartPrimitives")
    if cmd is None:
        errors.append("command 'PartDesign_PartPrimitives' is not registered in the command manager")
    else:
