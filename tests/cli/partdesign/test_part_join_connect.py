"""CLI test for PartDesign_PartJoinConnect command.

Bridge to Part command (dialog: Join connect)
...
COMMAND_ID = "PartDesign_PartJoinConnect"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartJoinConnect")
    if cmd is None:
        errors.append("command 'PartDesign_PartJoinConnect' is not registered in the command manager")
    else:
