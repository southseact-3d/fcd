"""CLI test for PartDesign_PartOffset2D command (bridge to Part_Offset2D).

Verifies that a Part::Offset2D object can be created with the expected TypeId.
"""
from __future__ import annotations

COMMAND_ID = "PartDesign_PartOffset2D"
TIMEOUT = 30
...
doc = App.newDocument("test_partdesign_partoffset2d")
errors = []
...
try:
    ...
    if obj is None:
        ...
        if obj.TypeId != "Part::Offset2D":
            errors.append(f"wrong TypeId: {obj.TypeId} (expected Part::Offset2D)")
        doc.recompute()
