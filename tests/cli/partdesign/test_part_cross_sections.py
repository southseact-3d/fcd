"""CLI test for PartDesign_PartCrossSections command.

Bridge to Part command (dialog: Cross sections)
...
COMMAND_ID = "PartDesign_PartCrossSections"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartCrossSections")
    if cmd is None:
        errors.append("command 'PartDesign_PartCrossSections' is not registered in the command manager")
    else:
