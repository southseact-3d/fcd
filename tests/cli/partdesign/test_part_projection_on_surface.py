"""CLI test for PartDesign_PartProjectionOnSurface command.

Bridge to Part command (dialog: Projection on surface)
...
COMMAND_ID = "PartDesign_PartProjectionOnSurface"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartProjectionOnSurface")
    if cmd is None:
        errors.append("command 'PartDesign_PartProjectionOnSurface' is not registered in the command manager")
    else:
