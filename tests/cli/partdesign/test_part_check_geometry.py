"""CLI test for PartDesign_PartCheckGeometry command.

Bridge to Part command (dialog: Check geometry)
...
COMMAND_ID = "PartDesign_PartCheckGeometry"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartCheckGeometry")
    if cmd is None:
        errors.append("command 'PartDesign_PartCheckGeometry' is not registered in the command manager")
    else:
