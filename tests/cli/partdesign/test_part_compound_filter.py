"""CLI test for PartDesign_PartCompoundFilter command.

Bridge to Part command (dialog: Compound filter)
...
COMMAND_ID = "PartDesign_PartCompoundFilter"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartCompoundFilter")
    if cmd is None:
        errors.append("command 'PartDesign_PartCompoundFilter' is not registered in the command manager")
    else:
