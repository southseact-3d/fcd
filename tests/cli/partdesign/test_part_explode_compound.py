"""CLI test for PartDesign_PartExplodeCompound command.

Bridge to Part command (dialog: Explode compound)
...
COMMAND_ID = "PartDesign_PartExplodeCompound"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartExplodeCompound")
    if cmd is None:
        errors.append("command 'PartDesign_PartExplodeCompound' is not registered in the command manager")
    else:
