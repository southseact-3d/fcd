"""CLI test for PartDesign_Migrate command.

verifies Migrate command runs on a fresh doc
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_Migrate")
    if cmd is None:
        errors.append("command 'PartDesign_Migrate' is not registered in the command manager")
    else:
