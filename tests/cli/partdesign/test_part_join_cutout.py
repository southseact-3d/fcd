"""CLI test for PartDesign_PartJoinCutout command.

Bridge to Part command (dialog: Join cutout)
...
COMMAND_ID = "PartDesign_PartJoinCutout"
TIMEOUT = 30
...
try:
    ...
    cmd = cm.getCommandByName("PartDesign_PartJoinCutout")
    if cmd is None:
        errors.append("command 'PartDesign_PartJoinCutout' is not registered in the command manager")
    else:
